\documentclass[12pt]{article}
\usepackage[utf8]{vietnam}
\usepackage[english]{babel}
 \usepackage{tabularray}
\usepackage{longtable}
\usepackage{pifont}
\usepackage{hyperref}
\usepackage{enumitem}
%\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{3}
\usepackage[dvipsnames]{xcolor}
\usepackage[left=2.55cm,right=2.5cm,top=2cm,bottom=2cm]{geometry} 
\setlength{\parindent}{30pt}
\usepackage{graphicx}
\usepackage{wrapfig}
%\usepackage{wallpaper}
%\usepackage[firstpage]{draftwatermark} 
\usepackage{xcolor}
\usepackage{tikz} 
\usepackage{scrextend}
\usepackage{amssymb}
\usepackage{indentfirst}
\usepackage{float}
\usepackage{array}
\usepackage{tabularx}
\usepackage{biblatex}
\usepackage{listings}
\usetikzlibrary{calc}
\addbibresource{ref.bib}
\usepackage{hyperref}
\usepackage{titlesec}
\titlespacing{\subsection}{0pt}{1pt}{0pt}

\begin{document}
\begin{center}
    \color{BlueViolet}
    
    \textbf{\newline\newline}
    \vspace{6pt}
    \large
    \textbf{\\TRƯỜNG ĐẠI HỌC KHOA HỌC TỰ NHIÊN}
    
    \vspace{6pt}
    \large
    \textbf{\\KHOA CÔNG NGHỆ THÔNG TIN}
\end{center}

\begin{center}
    \includegraphics[scale=0.25]{Logo_HCMUS/Logo_HCMUS.png}
    \color{BlueViolet}
    
   \large
   \textbf{HOMEWORK – AI-FIRST EDITION}
    \vspace{25pt}\\

    \color{BlueViolet}
    \textbf{GIẢNG VIÊN:} 
    \color{Black}
    \\MSc. Trần Thị Bích Hạnh
    \\MSc. Trương Phước Lộc
    \\MSc. Hồ Tuấn Thanh
    \vspace{15pt}\\


    \color{BlueViolet}
    \textbf{THÀNH VIÊN:}
    \color{Black}
     \\{Trần Minh Quang - 23127464}
\end{center}

\begin{tikzpicture}[remember picture,overlay,inner sep=0,outer sep=0]
     \draw[blue!70!black,line width=4pt] ([xshift=-1.5cm,yshift=-2cm]current page.north east) coordinate (A)--([xshift=1.5cm,yshift=-2cm]current page.north west) coordinate(B)--([xshift=1.5cm,yshift=2cm]current page.south west) coordinate (C)--([xshift=-1.5cm,yshift=2cm]current page.south east) coordinate(D)--cycle;

     \draw ([yshift=0.5cm,xshift=-0.5cm]A)-- ([yshift=0.5cm,xshift=0.5cm]B)--
     ([yshift=-0.5cm,xshift=0.5cm]B) --([yshift=-0.5cm,xshift=-0.5cm]B)--([yshift=0.5cm,xshift=-0.5cm]C)--([yshift=0.5cm,xshift=0.5cm]C)--([yshift=-0.5cm,xshift=0.5cm]C)-- ([yshift=-0.5cm,xshift=-0.5cm]D)--([yshift=0.5cm,xshift=-0.5cm]D)--([yshift=0.5cm,xshift=0.5cm]D)--([yshift=-0.5cm,xshift=0.5cm]A)--([yshift=-0.5cm,xshift=-0.5cm]A)--([yshift=0.5cm,xshift=-0.5cm]A);


     \draw ([yshift=-0.3cm,xshift=0.3cm]A)-- ([yshift=-0.3cm,xshift=-0.3cm]B)--
     ([yshift=0.3cm,xshift=-0.3cm]B) --([yshift=0.3cm,xshift=0.3cm]B)--([yshift=-0.3cm,xshift=0.3cm]C)--([yshift=-0.3cm,xshift=-0.3cm]C)--([yshift=0.3cm,xshift=-0.3cm]C)-- ([yshift=0.3cm,xshift=0.3cm]D)--([yshift=-0.3cm,xshift=0.3cm]D)--([yshift=-0.3cm,xshift=-0.3cm]D)--([yshift=0.3cm,xshift=-0.3cm]A)--([yshift=0.3cm,xshift=0.3cm]A)--([yshift=-0.3cm,xshift=0.3cm]A);

\end{tikzpicture}
\newpage
\renewcommand{\contentsname}{\textbf{Mục lục}}
\tableofcontents

\newpage
\input{Content/requirement 1}
\vspace{1cm}
\input{Content/requirement 2}
\vspace{1cm}
\input{Content/requirement 3}
\vspace{1cm}
\input{Content/AI Audit section}
\vspace{1cm}
\input{Content/AI Critique}
\vspace{1cm}
\input{Content/AI Mandatory Disclosure}
\vspace{1cm}
\newpage
\section{Self-Assessment}

\textit{The AI-first rubric below replaces the previous rubric. Total = 100 points. Every HW has an `AI Compliance' column worth 15--20 points -- missing the Audit Report / prompt log / critique forfeits the ENTIRE column.}

\vspace{0.5cm}

\begin{center}
\begin{tabular}{|>{\bfseries}l|p{7cm}|c|c|}
\hline
\textbf{No.} & \textbf{Criteria} & \textbf{Grade} & \textbf{Self-Assessed Grade} \\
\hline
1 & Job Market 2026+ (10 jobs $\times$ 3 pts + AI Impact) & 40 &  \\
\hline
2 & Software Defects 2022--2026 (20 defects) & 20 &  \\
\hline
3 & Physical-product test design (15 TCs + 5 videos) & 25 &  \\
\hline
AI-1 & {[AI-02]} AI Audit Report (5-section) attached & 8 &  \\
\hline
AI-2 & AI Critique 200--300 words + {[AI-03]} Disclosure attached & 4 &  \\
\hline
AI-3 & {[AI-05]} Checklist signed + anti-cheat artifacts & 3 &  \\
\hline
 & \textbf{Total} & \textbf{100} &  \\
\hline
\end{tabular}
\end{center}

\end{document}